...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($number) || empty($price) || empty($location) || empty($Cimage) || empty($additionalImages)||empty($additional_inf)||empty($room_num)||empty($room_type)) {
        ...
        // Database insertion
        $sql = "INSERT INTO product (coverpage,location,phone_number,price ,other_image,room_type,room_num,other_info)
                VALUES ('$cover_page',  '$location','$number','$price','$additional_images','$room_type','$room_num','$additional_inf')";
